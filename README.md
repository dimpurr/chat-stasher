...
- `cache` — reports this machine's conversation-body cache: where it lives, the quota it must stay under (`[cache] max_bytes`, default 2 GiB) and how much of it is in use; `cache clear` deletes every cached block. The cache holds each destination's own **ciphertext**, is disposable — losing it costs only speed — and is never consulted by `verify`, `export`, `dest-init`, `push` or `read --all-machines`, which read to prove the destination itself is intact. (`crates/chat-stasher/src/main.rs:1046-1060`; the config section is `crates/chat-stasher/src/config.rs:177-201`.)
...
- `status` reads the local harness locations known to the registry and prints IDs, paths, sizes, mtimes, and flags; it does not print session content. (`crates/chat-stasher/src/main.rs:7248-7439,7556-7847`.)
