...
(`apps/extension/lib/popup-view.ts:843-856`).
...
`apps/extension/entrypoints/background.ts:500-507`).
...
- **🔴 Turning on backfill still does not mean every platform's history will
  ...
  **in the claude.ai page**, over the same channel the backfill fetches through,
  ...
  (`apps/extension/entrypoints/background.ts:1392-1456`). Perplexity **only lists
