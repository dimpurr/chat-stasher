...
(`apps/extension/lib/contract.ts:317,385,401,442,500,627,714`).
...
- **Passive capture** (on by default): the conversation you are currently
  ...
  count (`apps/extension/lib/contract.ts:270-716`).
  ...
  (`apps/extension/lib/contract.ts:320-386`) — and it recognizes the session id
  from the page URL, the `/search/<slug>` the thread is open at
  (`apps/extension/lib/contract.ts:373-376`). The **conversation-list** route is
  ...
  (`apps/extension/lib/contract.ts:949-977`) — but this is still a conclusion
...
conversation (`apps/extension/lib/backfill/enumerate.ts:2215-2242`). The evidence
...
(`apps/extension/lib/backfill/enumerate.ts:2199-2213`). What the check cannot
...
(`apps/extension/lib/backfill/enumerate.ts:2409-2423`), but this plan's
...
(`apps/extension/lib/contract.ts:338-353`), but the parameters the sources give
...
(`apps/extension/lib/backfill/enumerate.ts:2397-2406`, `:2409-2423`). We will not guess a
...
`apps/extension/lib/contract.ts:270-716`, a separate matter from backfill.)
...
- **🔴 Turning on backfill still does not mean every platform's history will
  ...
  **in the claude.ai page**, over the same channel the backfill fetches through,
  ...
  (`apps/extension/entrypoints/background.ts:941-963`). Perplexity **only lists
  conversations, saving none of their content**. See section 1.1 for
  the list and the detailed explanation (list from
  `apps/extension/lib/backfill/enumerate.ts:3866-3893`). The
