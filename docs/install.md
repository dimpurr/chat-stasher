...
(`apps/extension/lib/popup-view.ts:835-848`).
...
`apps/extension/entrypoints/background.ts:496-503`).
...
- **🔴 Turning on backfill still does not mean every platform's history will
  ...
  **in the claude.ai page**, over the same channel the backfill fetches through,
  ...
  (`apps/extension/entrypoints/background.ts:1224-1285`). Perplexity **only lists
