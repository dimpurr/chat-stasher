...
read-only (`crates/chat-stasher/src/main.rs:513`).
...
(`crates/chat-stasher/src/main.rs:649-691`). The protocol both sides implement
...
left alone, skipped or removed, absolutely (`crates/chat-stasher/src/main.rs:626-648`).
...
(`crates/chat-stasher/src/main.rs:2813-2826`); without it, an unattended
...
it does anything else (`crates/chat-stasher/src/main.rs:2849-2873`). An
...
to `~/.ssh/known_hosts` (`crates/chat-stasher/src/main.rs:2828-2837`;
...
silently doing nothing (`crates/chat-stasher/src/main.rs:2816-2824`).
...
does not (`crates/chat-stasher/src/main.rs:5889-5890`). This is the
...
(`crates/chat-stasher/src/main.rs:5670-5671`). These are the conclusions defined
...
(`crates/chat-stasher/src/main.rs:5880-5890`):
...
(`crates/chat-stasher/src/main.rs:5738-5745`). So "the command errored"
...
- **There is no `restore` (bulk recovery) command. Not in phase one.** The
  ...
  (`crates/chat-stasher/src/main.rs:44-851`). What you can do is `read`, which
