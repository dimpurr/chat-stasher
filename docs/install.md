...
read-only (`crates/chat-stasher/src/main.rs:696`).
...
(`crates/chat-stasher/src/main.rs:832-874`). The protocol both sides implement
...
left alone, skipped or removed, absolutely (`crates/chat-stasher/src/main.rs:809-831`).
...
(`crates/chat-stasher/src/main.rs:262-294,356-397,410-459`).
...
installed path outside `target/` (`crates/chat-stasher/src/main.rs:189-256`).
The generated template wraps a `run-once` command
(`crates/chat-stasher/src/main.rs:189-256`).

`run-once` is one complete collect-and-push pass; it exits when done, and
repeated invocation is safe (`crates/chat-stasher/src/main.rs:151-188`).
...
(`crates/chat-stasher/src/main.rs:318-320`).
...
(`crates/chat-stasher/src/main.rs:398-409`).
...
(`crates/chat-stasher/src/main.rs:562-642`).
...
- **There is no `restore` command — nothing puts a session back into a
  ...
  has no `restore` entry (`crates/chat-stasher/src/main.rs:144-1087`). Getting
  content *out* does have a bulk path: `export --out <dir>` writes every session
  a time window selects to files in one command
  (`crates/chat-stasher/src/main.rs:562-642`), and `read` dumps **one**
  conversation to standard output at a time
  (`crates/chat-stasher/src/main.rs:353-397`). Restoring = for now you have to
...
- **`schedule` render-only mode does not install the timer**; use the explicit
  ...
  remains render-only in this command (`crates/chat-stasher/src/main.rs:189-256`).
...
point — collect, push, exit (`crates/chat-stasher/src/main.rs:151-188`). It does
