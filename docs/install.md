...
(`apps/extension/lib/popup-view.ts:831-844`).
...
`apps/extension/entrypoints/background.ts:493-500`).
...
- **🔴 Turning on backfill still does not mean every platform's history will
  ...
  **in the claude.ai page**, over the same channel the backfill fetches through,
  ...
  (`apps/extension/entrypoints/background.ts:1172-1233`). Perplexity **only lists
