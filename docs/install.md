...
- **Passive capture** (on by default): the conversation you are currently
  ...
  (`apps/extension/lib/contract.ts:1040-1068`) — but this is still a conclusion
...
`apps/extension/entrypoints/background.ts:718-720`). That page does not have to
...
conversation (`apps/extension/lib/backfill/enumerate.ts:2748-2775`). The evidence
...
(`apps/extension/lib/backfill/enumerate.ts:2732-2746`). What the check cannot
...
(`apps/extension/lib/backfill/enumerate.ts:2909-2920`). What W84 observed, and
...
with the next conversation (`apps/extension/lib/backfill/enumerate.ts:2257-2301`);
...
`apps/extension/entrypoints/background.ts:504-511`).
...
(`crates/chat-stasher/src/main.rs:3859-3872`); without it, an unattended
...
it does anything else (`crates/chat-stasher/src/main.rs:3895-3919`). An
...
to `~/.ssh/known_hosts` (`crates/chat-stasher/src/main.rs:3874-3883`;
...
silently doing nothing (`crates/chat-stasher/src/main.rs:3862-3870`).
...
- **🔴 Turning on backfill still does not mean every platform's history will
  ...
  **in the claude.ai page**, over the same channel the backfill fetches through,
  ...
  (`apps/extension/entrypoints/background.ts:1481-1562`). Perplexity now lists
  ...
  `apps/extension/lib/backfill/enumerate.ts:4580-4607`) is covered.
