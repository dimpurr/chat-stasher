...
(`apps/extension/lib/popup-view.ts:823-836`).
...
`apps/extension/entrypoints/background.ts:482-489`).
...
- **🔴 Turning on backfill still does not mean every platform's history will
  ...
  **in the claude.ai page**, over the same channel the backfill fetches through,
  ...
  (`apps/extension/entrypoints/background.ts:1161-1222`). Perplexity **only lists
