...
- **Running on a site is not the same as backing up your history there.** The
  ...
  **ChatGPT**, **DeepSeek**, **Gemini**, **Grok**, **Kimi** and **Claude** — on none of the
  ...
  only, and attached to its two RPCs and nothing else (`apps/extension/lib/platform-auth.ts:513-575`).
...
1. **Capture.** A content script, injected only on a fixed list of chat origins,
   ...
   **One exception, on ChatGPT.** When you move between conversations inside
   ...
   `apps/extension/entrypoints/dw-bridge.content.ts:598-624`). That request —
   ...
   **A second token, on Kimi, and it is read from the page rather than requested.**
   ...
   conversations” (`apps/extension/lib/platform-auth.ts:216-248`,
...
- **A DeepSeek halt record says what the response looked like, and only that.**
  ...
  **No value from the response is ever in it** — not an id, not a title, not a
  ...
  named per object
  (`apps/extension/lib/backfill/enumerate.ts:1231-1299`).
- The `<scope>` part of that key is your **account identifier on that platform**
  ...
  (`apps/extension/entrypoints/background.ts:905-948` — the identity itself is
  read by `apps/extension/lib/contract.ts:1053-1069`; the `default` fallback is on
  the `||` at `apps/extension/entrypoints/background.ts:948`). It is used to
  ...
  (`apps/extension/entrypoints/background.ts:812-892`).
- **On claude.ai the scope is not read from a response body: it is the
  ...
  **in the claude.ai page**, over the channel the backfill already uses, and only
  ...
  `apps/extension/entrypoints/background.ts:794-815`). The sentinel
  ...
  (`apps/extension/entrypoints/background.ts:1052-1092`). A backfill for a *second*
  ...
  `apps/extension/entrypoints/background.ts:674-704`). Collapsing a title cannot
  ...
  `apps/extension/entrypoints/background.ts:1052-1092`). The host archive is
  ...
  `apps/extension/lib/popup-view.ts:1377-1439`).
...
(`apps/extension/lib/backfill/enumerate.ts:4048-4075`;
`apps/extension/lib/backfill/engine.ts:1334-1386`.) The
...
`apps/extension/lib/backfill/enumerate.ts:2634`), and on Kimi one body request —
the same call its own page makes when you open a conversation, carrying the page's
own token as described above (`apps/extension/lib/backfill/enumerate.ts:2782-2844`).
...
(`apps/extension/lib/backfill/enumerate.ts:3362-3374`).
...
(`apps/extension/lib/backfill/enumerate.ts:3221-3349`; `apps/extension/lib/backfill/engine.ts:1334-1386`).
...
half-there (`apps/extension/lib/backfill/engine.ts:1438-1473`). Grok
