...
- **On claude.ai the scope is not read from a response body: it is the
  ...
  **in the claude.ai page**, over the channel the backfill already uses, and only
  ...
  organization that does not exist (`apps/extension/lib/backfill/engine.ts:1223-1235`).
...
(`apps/extension/lib/backfill/engine.ts:475`). That produces a request pattern
...
`apps/extension/lib/backfill/engine.ts:1799-1851`.) The
...
(`apps/extension/lib/backfill/enumerate.ts:3344-3472`; `apps/extension/lib/backfill/engine.ts:1799-1851`).
...
half-there (`apps/extension/lib/backfill/engine.ts:1903-1938`). Grok
...
(`apps/extension/lib/backfill/engine.ts:1325-1372`). And because no source says
