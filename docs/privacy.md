...
is, the conversation itself (`apps/extension/entrypoints/background.ts:148-176`;
...
- The `<scope>` part of that key is your **account identifier on that platform**
  ...
  (`apps/extension/entrypoints/background.ts:1203-1246` — the identity itself is
  read by `apps/extension/lib/contract.ts:1053-1069`; the `default` fallback is on
  the `||` at `apps/extension/entrypoints/background.ts:1246`). It is used to
  ...
  (`apps/extension/entrypoints/background.ts:966-987`, `:1116-1190`).
- **On claude.ai the scope is not read from a response body: it is the
  ...
  **in the claude.ai page**, over the channel the backfill already uses, and only
  ...
  `apps/extension/entrypoints/background.ts:925-969`). A record an **earlier**
  ...
  (`apps/extension/entrypoints/background.ts:1393-1457`). A backfill for a *second*
  ...
  `apps/extension/entrypoints/background.ts:749-779`). Collapsing a title cannot
  ...
  `apps/extension/entrypoints/background.ts:1393-1457`). The host archive is
