...
is, the conversation itself (`apps/extension/entrypoints/background.ts:135-163`;
...
| `cs_backfill_lasttick_v1` | The trace of the most recent backfill alarm wake: when it was, whether it ran, the named outcome, and how many backfill targets were registered. 🔴 It also carries **how that tick ended** — the run's own stop reason (`stopped`), the halt it left behind (`halted`) and that halt's `detail`, or, for a tick that stopped before making any request, that tick's own named outcome. And whether that tick **swept open tabs** for a live page the registry had lost (`tabSweep`): `null` if it never swept, `{ looked: false }` if it could not list tabs, `{ looked: true, queried, pruned, pinged, registered, deferred }` if it did — counts only, so "we looked and found nothing" stays distinct from "we never looked", and a sweep that hit its ping cap (`deferred > 0`) stays distinct from one that pinged everything it wanted to. Metadata only: reason codes, counts and timestamps. The one free-text field is the halt's `detail`, and by construction it names storage keys, paths, HTTP statuses and counts — never a conversation id, title, or body. One record, overwritten by the next wake. | `apps/extension/lib/backfill/alarm.ts:582-673`; `apps/extension/entrypoints/background.ts:1223-1279` |
| `cs_hook_v1:<origin>`, `cs_hook_declined_v1` | A page's own report about its capture hook: one record per origin, holding each observation and when it was made. 🔴 And the one report that was **received and not recorded**, with the check that refused it, the origin and observation when they are known, how many times in a row the same refusal has repeated, and when. Metadata only: reason codes, a count, an origin string, timestamps; no URL path, no conversation id, no body, no token. Unlike the per-origin records, the declined one is a single record, overwritten by the next decline. | `apps/extension/lib/hook-status.ts:69-101`, `:226-355`; `apps/extension/entrypoints/background.ts:1409-1425`, `:1451-1467` |
...
- The `<scope>` part of that key is your **account identifier on that platform**
  ...
  (`apps/extension/entrypoints/background.ts:859-883` — the identity itself is
  read by `apps/extension/lib/contract.ts:1051-1067`; the `default` fallback is on
  the `||` at `apps/extension/entrypoints/background.ts:904`). It is used to
  ...
  (`apps/extension/entrypoints/background.ts:778-850`).
- **On claude.ai the scope is not read from a response body: it is the
  ...
  **in the claude.ai page**, over the channel the backfill already uses, and only
  ...
  `apps/extension/entrypoints/background.ts:760-776`). The sentinel
  ...
  (`apps/extension/entrypoints/background.ts:1004-1026`). A backfill for a *second*
...
(`apps/extension/lib/backfill/enumerate.ts:3063-3191`; `apps/extension/lib/backfill/engine.ts:1302-1354`).
