...
- **Running on a site is not the same as backing up your history there.** The
  ...
  **ChatGPT**, **DeepSeek**, **Gemini**, **Grok** and **Kimi** — on none of the
  five has a complete backfill been observed in a real browser, and for DeepSeek,
  Gemini, Grok and Kimi we have **not verified** whether a long conversation comes
  back complete (Gemini is the one of the three that pages; the other two do not,
  and say so). Grok is the least verified of the five: its routes come from reading
  public open-source implementations rather than from a logged-in grok.com session,
  and one conversation costs two requests. Kimi's routes *were* measured in a
  ...
  only, and attached to its two RPCs and nothing else (`apps/extension/lib/platform-auth.ts:355-417`).
  On **Perplexity** it lists your conversations and saves **none of their
  content**; on Claude it does nothing at all. See
...
1. **Capture.** A content script, injected only on a fixed list of chat origins,
   ...
   (`apps/extension/lib/page-hook.ts:422`, `:461`, `:468-485`). Only responses
   matching a known platform route are kept
   (`apps/extension/lib/contract.ts:130-576`, `:700-728`).
   **One exception, on ChatGPT.** When you move between conversations inside
   ...
   `apps/extension/entrypoints/dw-bridge.content.ts:296-322`). That request —
   ...
   other request (`apps/extension/lib/platform-auth.ts:59-69`, `:99-118`).
   **A second token, on Kimi, and it is read from the page rather than requested.**
   ...
   `apps/extension/entrypoints/dw-bridge.content.ts:200-206`).
2. **Queue on your machine.** The extension writes that text, as a JSON bundle,
   ...
   (`apps/extension/entrypoints/background.ts:186-203`).
...
- **Check the network tab.** Open your browser's developer tools on a chat page
  ...
  (`apps/extension/lib/page-hook.ts:461`, `:468-485`). The one feature that does
...
is, the conversation itself (`apps/extension/entrypoints/background.ts:113-141`;
...
- The `<scope>` part of that key is your **account identifier on that platform**
  ...
  (`apps/extension/entrypoints/background.ts:631-650` — the identity itself is
  read by `apps/extension/lib/contract.ts:911-927`; the `default` fallback is on
  the `||` at `apps/extension/entrypoints/background.ts:649`). It is used to
  ...
  (`apps/extension/entrypoints/background.ts:589-620`).
...
(`apps/extension/lib/backfill/engine.ts:299`). That produces a request pattern
...
(`apps/extension/lib/backfill/engine.ts:85-88`).
...
you have open). On **Claude** backfill issues no requests at all.
(`apps/extension/lib/backfill/enumerate.ts:2640-2650`, `:2654-2660`, `:2662-2667`;
`apps/extension/lib/backfill/engine.ts:1079-1131`.) The
...
`apps/extension/lib/backfill/enumerate.ts:1650`), and on Kimi one body request —
the same call its own page makes when you open a conversation, carrying the page's
own token as described above (`apps/extension/lib/backfill/enumerate.ts:1798-1860`).
...
(`apps/extension/lib/backfill/enumerate.ts:2237-2367`; `apps/extension/lib/backfill/engine.ts:1079-1131`).
...
- `https://chat.deepseek.com` (`apps/extension/lib/contract.ts:133`)
- `https://www.perplexity.ai` (`apps/extension/lib/contract.ts:245`)
- `https://chatgpt.com`, `https://chat.openai.com` (`apps/extension/lib/contract.ts:249`)
- `https://gemini.google.com` (`apps/extension/lib/contract.ts:265`)
- `https://claude.ai` (`apps/extension/lib/contract.ts:306`)
- `https://www.kimi.com` (`apps/extension/lib/contract.ts:372`)
- `https://grok.com` (`apps/extension/lib/contract.ts:494`)

The list the browser is given is derived mechanically from that table
(`apps/extension/lib/contract.ts:579-581`), so the sites the extension can run
...
(`apps/extension/lib/contract.ts:679-698`, `:791-799`). A body over 16 MiB is not
captured, and the page console says so rather than dropping it silently
(`apps/extension/lib/contract.ts:601`; `apps/extension/lib/page-hook.ts:241`). No shipped
platform row reads WebSocket frames; every row sets that switch to `false`
explicitly (`apps/extension/lib/contract.ts:177`, `:245`, `:261`, `:302`, `:360`,
`:487`, `:574`).
...
|---|---|
| **ChatGPT**, **DeepSeek**, **Gemini**, **Grok**, **Kimi** | Lists your conversations **and fetches their content**, one conversation at a time, handing it to the host (`apps/extension/lib/backfill/enumerate.ts:2640-2650`). All five are **implemented, not yet observed completing a backfill in a real browser**. For DeepSeek, Grok and Kimi we have **not verified** whether a long conversation comes back complete, because the extension does not page those endpoints (`apps/extension/lib/backfill/enumerate.ts:1458-1471`). **Gemini does page**, so for it the completeness question is answered by following the token to the end; what bounds it instead is the 20-page cap, past which the conversation is refused rather than archived in part (`apps/extension/lib/backfill/engine.ts:1079-1131`). Kimi's routes, by contrast, **were** measured in a logged-in session (2026-09-14) and its one body request carries your page's own login token (`apps/extension/lib/platform-auth.ts:214-246`); whether a **long** Kimi conversation comes back complete is **not verified**, and a response that says it holds only part of a conversation is recorded as a failure rather than archived as a whole one (`apps/extension/lib/backfill/engine.ts:1183-1218`). Grok is the least verified: its routes were read from public open-source implementations rather than measured in a logged-in session, and **each conversation costs two requests** — a skeleton call, then a content call built only from the ids that skeleton named (`apps/extension/lib/backfill/enumerate.ts:1713-1774`). |
| **Perplexity** | Lists your conversations and **saves none of their content** — **nothing is delivered or queued, so this history is not backed up** (`apps/extension/lib/backfill/enumerate.ts:2654-2660`). |
| **Claude** | Nothing; the leg stops before issuing any request (`apps/extension/lib/backfill/enumerate.ts:2402-2405`, `:2662-2667`). |
...
half-there (`apps/extension/lib/backfill/engine.ts:1183-1218`). Grok
...
(`apps/extension/lib/backfill/engine.ts:698-740`). And because no source says
...
`apps/extension/wxt.config.ts:87`). The one thing Chrome does tell you at
