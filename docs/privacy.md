...
- **On claude.ai the scope is not read from a response body: it is the
  ...
  **in the claude.ai page**, over the channel the backfill already uses, and only
  ...
  organization that does not exist (`apps/extension/lib/backfill/engine.ts:1074-1085`).
...
(`apps/extension/lib/backfill/engine.ts:437`). That produces a request pattern
...
`apps/extension/lib/backfill/engine.ts:1606-1658`.) The
...
(`apps/extension/lib/backfill/enumerate.ts:3063-3191`; `apps/extension/lib/backfill/engine.ts:1606-1658`).
...
half-there (`apps/extension/lib/backfill/engine.ts:1710-1745`). Grok
...
(`apps/extension/lib/backfill/engine.ts:1165-1212`). And because no source says
