...
is, the conversation itself (`apps/extension/entrypoints/background.ts:140-168`;
...
- The `<scope>` part of that key is your **account identifier on that platform**
  ...
  (`apps/extension/entrypoints/background.ts:1014-1057` — the identity itself is
  read by `apps/extension/lib/contract.ts:1053-1069`; the `default` fallback is on
  the `||` at `apps/extension/entrypoints/background.ts:1057`). It is used to
  ...
  (`apps/extension/entrypoints/background.ts:905-1001`).
- **On claude.ai the scope is not read from a response body: it is the
  ...
  **in the claude.ai page**, over the channel the backfill already uses, and only
  ...
  `apps/extension/entrypoints/background.ts:864-908`). A record an **earlier**
  ...
  (`apps/extension/entrypoints/background.ts:1161-1222`). A backfill for a *second*
  ...
  `apps/extension/entrypoints/background.ts:730-760`). Collapsing a title cannot
  ...
  `apps/extension/entrypoints/background.ts:1161-1222`). The host archive is
  ...
  `apps/extension/lib/popup-view.ts:1550-1612`).
