...
- The `<scope>` part of that key is your **account identifier on that platform**
  ...
  (`apps/extension/entrypoints/background.ts:928-971` — the identity itself is
  read by `apps/extension/lib/contract.ts:1053-1069`; the `default` fallback is on
  the `||` at `apps/extension/entrypoints/background.ts:971`). It is used to
  ...
  (`apps/extension/entrypoints/background.ts:835-915`).
- **On claude.ai the scope is not read from a response body: it is the
  ...
  **in the claude.ai page**, over the channel the backfill already uses, and only
  ...
  `apps/extension/entrypoints/background.ts:809-830`). A record an **earlier**
  ...
  build that saw it. The sentinel
  ...
  organization that does not exist (`apps/extension/lib/backfill/engine.ts:900-911`).
  ...
  (`apps/extension/entrypoints/background.ts:1075-1115`). A backfill for a *second*
  ...
  `apps/extension/entrypoints/background.ts:1075-1115`). The host archive is
  ...
  `apps/extension/lib/popup-view.ts:1407-1469`).
...
(`apps/extension/lib/backfill/engine.ts:434`). That produces a request pattern
...
(`apps/extension/lib/backfill/engine.ts:88-91`).
...
`apps/extension/lib/backfill/engine.ts:1389-1441`.) The
...
(`apps/extension/lib/backfill/enumerate.ts:3063-3191`; `apps/extension/lib/backfill/engine.ts:1389-1441`).
...
half-there (`apps/extension/lib/backfill/engine.ts:1493-1528`). Grok
...
(`apps/extension/lib/backfill/engine.ts:973-1018`). And because no source says
