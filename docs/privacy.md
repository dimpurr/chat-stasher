...
- The `<scope>` part of that key is your **account identifier on that platform**
  ...
  (`apps/extension/entrypoints/background.ts:1025-1068` — the identity itself is
  read by `apps/extension/lib/contract.ts:1053-1069`; the `default` fallback is on
  the `||` at `apps/extension/entrypoints/background.ts:1068`). It is used to
  ...
  (`apps/extension/entrypoints/background.ts:916-1012`).
- **On claude.ai the scope is not read from a response body: it is the
  ...
  **in the claude.ai page**, over the channel the backfill already uses, and only
  ...
  `apps/extension/entrypoints/background.ts:875-919`). A record an **earlier**
  ...
  (`apps/extension/entrypoints/background.ts:1172-1233`). A backfill for a *second*
  ...
  `apps/extension/entrypoints/background.ts:741-771`). Collapsing a title cannot
  ...
  `apps/extension/entrypoints/background.ts:1172-1233`). The host archive is
  ...
  `apps/extension/lib/popup-view.ts:1581-1643`).
