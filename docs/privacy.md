...
- **Running on a site is not the same as backing up your history there.** The
  ...
  **ChatGPT**, **DeepSeek**, **Gemini**, **Grok**, **Kimi** and **Claude** — on none of the
  ...
  only, and attached to its two RPCs and nothing else (`apps/extension/lib/platform-auth.ts:577-639`).
...
- **On claude.ai the scope is not read from a response body: it is the
  ...
  **in the claude.ai page**, over the channel the backfill already uses, and only
  ...
  organization that does not exist (`apps/extension/lib/backfill/engine.ts:889-900`).
  ...
  `apps/extension/lib/popup-view.ts:1406-1468`).
...
(`apps/extension/lib/backfill/engine.ts:471`). That produces a request pattern
...
(`apps/extension/lib/backfill/enumerate.ts:4171-4198`;
`apps/extension/lib/backfill/engine.ts:1399-1451`.) The
...
`apps/extension/lib/backfill/enumerate.ts:2757`), and on Kimi one body request —
the same call its own page makes when you open a conversation, carrying the page's
own token as described above (`apps/extension/lib/backfill/enumerate.ts:2905-2967`).
...
(`apps/extension/lib/backfill/enumerate.ts:3485-3497`).
...
(`apps/extension/lib/backfill/enumerate.ts:3344-3472`; `apps/extension/lib/backfill/engine.ts:1399-1451`).
...
half-there (`apps/extension/lib/backfill/engine.ts:1503-1538`). Grok
...
(`apps/extension/lib/backfill/engine.ts:962-1007`). And because no source says
