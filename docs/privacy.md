...
- **A DeepSeek halt record says what the response looked like, and only that.**
  ...
  **No value from the response is ever in it** — not an id, not a title, not a
  ...
  named per object
  (`apps/extension/lib/backfill/enumerate.ts:1351-1419`).
...
(`apps/extension/lib/backfill/engine.ts:2296-2314`). That produces a request pattern
...
(`apps/extension/lib/backfill/enumerate.ts:4590-4621`;
`apps/extension/lib/backfill/engine.ts:2384-2457`.) The
...
`apps/extension/lib/backfill/enumerate.ts:3053`), and on Kimi one body request —
the same call its own page makes when you open a conversation, carrying the page's
own token as described above (`apps/extension/lib/backfill/enumerate.ts:3201-3263`).
...
(`apps/extension/lib/backfill/enumerate.ts:3781-3793`).
...
(`apps/extension/lib/backfill/enumerate.ts:3688-3816`; `apps/extension/lib/backfill/engine.ts:2413-2453`).
...
half-there (`apps/extension/lib/backfill/engine.ts:2613-2626`). Grok
