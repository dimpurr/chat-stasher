...
- **Running on a site is not the same as backing up your history there.** The
  ...
  **ChatGPT**, **DeepSeek**, **Gemini**, **Grok**, **Kimi** and **Claude** — on none of the
  ...
  only, and attached to its two RPCs and nothing else (`apps/extension/lib/platform-auth.ts:624-686`).
...
- **On claude.ai the scope is not read from a response body: it is the
  ...
  **in the claude.ai page**, over the channel the backfill already uses, and only
  ...
  organization that does not exist (`apps/extension/lib/backfill/engine.ts:1314-1326`).
...
(`apps/extension/lib/backfill/engine.ts:631`). That produces a request pattern
...
`apps/extension/lib/backfill/engine.ts:1890-1942`.) The
...
(`apps/extension/lib/backfill/enumerate.ts:3344-3472`; `apps/extension/lib/backfill/engine.ts:1890-1942`).
...
half-there (`apps/extension/lib/backfill/engine.ts:1994-2029`). Grok
...
(`apps/extension/lib/backfill/engine.ts:1416-1463`). And because no source says
