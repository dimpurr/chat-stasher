...
- **A halt record says what the response looked like, and only that.** When a
  response does not match the shape a parser recognises, the leg stops and the
  halt detail carries the *structure* of what arrived: the key names at the level
  that disagreed, the type of each, and array lengths. **No value from the
  response is ever in it** — not an id, not a title, not a message body, and not a
  fragment or a length of one; strings are reported as `string` and nothing more.
  ...
  named per object
  (`apps/extension/lib/backfill/enumerate.ts:1200-1245`).
...
(`apps/extension/lib/backfill/enumerate.ts:3866-3893`;
...
`apps/extension/lib/backfill/enumerate.ts:2452`), and on Kimi one body request —
the same call its own page makes when you open a conversation, carrying the page's
own token as described above (`apps/extension/lib/backfill/enumerate.ts:2600-2662`).
...
(`apps/extension/lib/backfill/enumerate.ts:3180-3192`).
...
(`apps/extension/lib/backfill/enumerate.ts:3039-3167`; `apps/extension/lib/backfill/engine.ts:1302-1354`).
