...
- The `<scope>` part of that key is your **account identifier on that platform**
  ...
  (`apps/extension/entrypoints/background.ts:858-882` — the identity itself is
  read by `apps/extension/lib/contract.ts:1051-1067`; the `default` fallback is on
  the `||` at `apps/extension/entrypoints/background.ts:903`). It is used to
  ...
  (`apps/extension/entrypoints/background.ts:777-849`).
- **On claude.ai the scope is not read from a response body: it is the
  ...
  **in the claude.ai page**, over the channel the backfill already uses, and only
  ...
  `apps/extension/entrypoints/background.ts:759-775`). The sentinel
  ...
  (`apps/extension/entrypoints/background.ts:1003-1025`). A backfill for a *second*
