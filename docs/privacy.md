...
- **On claude.ai the scope is not read from a response body: it is the
  ...
  **in the claude.ai page**, over the channel the backfill already uses, and only
  ...
  organization that does not exist (`apps/extension/lib/backfill/engine.ts:1525-1537`).
...
(`apps/extension/lib/backfill/engine.ts:2142-2160`). That produces a request pattern
...
(`apps/extension/lib/backfill/engine.ts:109-112`).
...
`apps/extension/lib/backfill/engine.ts:2230-2303`.) The
...
(`apps/extension/lib/backfill/enumerate.ts:3619-3747`; `apps/extension/lib/backfill/engine.ts:2259-2299`).
...
half-there (`apps/extension/lib/backfill/engine.ts:2459-2472`). Grok
...
(`apps/extension/lib/backfill/engine.ts:1733-1811`). And because no source says
