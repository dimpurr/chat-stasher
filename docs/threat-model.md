...
1. **The outbox window, before delivery.** The extension writes each captured
   ...
   (`apps/extension/entrypoints/background.ts:147-175`). It sits there,
...
(`apps/extension/entrypoints/background.ts:162-164`). It never leaves your
...
7. **Browser-side history backfill runs on seven platforms, recovers text on
   ...
   **ChatGPT**, **DeepSeek**, **Gemini**, **Grok**, **Kimi** and **Claude**
   ...
   `apps/extension/entrypoints/background.ts:924-968`). Kimi's routes, by contrast, were measured in a logged-in session,
