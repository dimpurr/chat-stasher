...
(`apps/extension/lib/backfill/engine.ts:1764-1773`, `:2253-2257`), which produces a
...
- **`push` refuses an unprovable empty snapshot.** If the stage holds neither
  ...
  (`crates/chat-stasher/src/main.rs:5326-5419`). It also fails closed when it
  cannot even establish stage safety
  (`crates/chat-stasher/src/main.rs:5298-5305`).
- **A destination that cannot be consulted is not an empty destination.**
  ...
  as many words (`crates/chat-stasher/src/main.rs:4030-4086`).
...
4. **There is no restore command.** The subcommands in this version are `init`,
  ...
  (`crates/chat-stasher/src/main.rs:130-1044`); **a command that puts sessions
   ...
   (`crates/chat-stasher/src/main.rs:332-334,5655-5773`). `export --out <dir>`
...
7. **Browser-side history backfill runs on seven platforms, recovers text on
   ...
   **ChatGPT**, **DeepSeek**, **Gemini**, **Grok**, **Kimi** and **Claude**
   ...
   rather than archived (`apps/extension/lib/backfill/engine.ts:2559-2572`).
   ...
   `apps/extension/lib/backfill/engine.ts:2359-2399`). On
