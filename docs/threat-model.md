...
(`crates/chat-stasher/src/main.rs:235-302`), the schedule itself is visible to
...
3. **The stage directory.** Sealed shards are ordinary files on disk before
   ...
   (`crates/chat-stasher/src/main.rs:303-307`).
...
5. **A directory you exported to.** `chat-stasher export --out <dir>` writes the
   ...
   into a directory you name (`crates/chat-stasher/src/main.rs:608-688`). Unlike
...
- **Harness session stores are opened read-only.** Every SQLite connection uses
  ...
  (`crates/chat-stasher/src/main.rs:367,444-445`).
- **`seal` refuses to rename files it cannot justify renaming.** It is gated by
  ...
  the old inode (`crates/chat-stasher/src/main.rs:778-810`).
...
- **`push` refuses an unprovable empty snapshot.** If the stage holds neither
  ...
  (`crates/chat-stasher/src/main.rs:5894-5990`). It also fails closed when it
  cannot even establish stage safety
  (`crates/chat-stasher/src/main.rs:5866-5873`).
- **A destination that cannot be consulted is not an empty destination.**
  ...
  as many words (`crates/chat-stasher/src/main.rs:4329-4385`).
...
4. **There is no restore command.** The subcommands in this version are `init`,
  ...
  (`crates/chat-stasher/src/main.rs:148-1135`); **a command that puts sessions
   ...
   (`crates/chat-stasher/src/main.rs:399-401,6226-6365`). `export --out <dir>`
   writes **many** sessions to files in one command, laid out as
   `<out>/<machine>/<harness>/<session-id>.jsonl`, and its directory is
   **plaintext** (`crates/chat-stasher/src/main.rs:608-688`) — see exposure 5
...
5. **Search is metadata-only.** `search` walks snapshot/index/tree objects and
   ...
   (`crates/chat-stasher/src/main.rs:546-576`). One qualification, because the
   ...
   **and** from "read it all but could not place every session in time", which
   ...
   (`crates/chat-stasher/src/main.rs:572-576`).
...
5. Run `verify` (`crates/chat-stasher/src/main.rs:456-494`) rather than assuming
