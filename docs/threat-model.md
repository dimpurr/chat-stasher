...
2. The extension writes that text, as a JSON bundle, into its **own IndexedDB
   ...
   `apps/extension/entrypoints/background.ts:269-286`).
3. The extension delivers the bundle to a **Native Messaging host** — the
   ...
   (`crates/chat-stasher/src/main.rs:655-705`).
...
(`crates/chat-stasher/src/main.rs:176-235`), the schedule itself is visible to
...
1. **The outbox window, before delivery.** The extension writes each captured
   ...
   (`apps/extension/entrypoints/background.ts:149-177`). It sits there,
...
3. **The stage directory.** Sealed shards are ordinary files on disk before
   ...
   (`crates/chat-stasher/src/main.rs:236-240`).
...
5. **A directory you exported to.** `chat-stasher export --out <dir>` writes the
   ...
   into a directory you name (`crates/chat-stasher/src/main.rs:541-621`). Unlike
...
`apps/extension/entrypoints/dw-bridge.content.ts:608-634`;
`apps/extension/lib/platform-auth.ts:101-120`). That is one extra request per
...
backfill paths and nothing else, and holds no copy (`apps/extension/lib/platform-auth.ts:258-295`);
...
and nothing else (`apps/extension/lib/platform-auth.ts:669-769`;
...
(`apps/extension/lib/backfill/engine.ts:1678-1687`, `:2137-2141`), which produces a
...
`GET /rest/thread/<slug>` per conversation — but it does not decide "whole" by
"the response came back with some body". It archives a conversation only when
the body itself declares there is no more: a 2026-09-23 logged-in probe measured
...
than being read as complete (`apps/extension/lib/backfill/enumerate.ts:2160-2204`).
...
directly observed — the rule refuses when the signal says more and treats the
response as whole when it says no more, and no complete Perplexity backfill has
been observed in a real browser. That other failure mode — a truncated body
stored whole while looking like success — is why DeepSeek's row is marked
...
(`apps/extension/lib/contract.ts:1040-1055`, `:1142-1158`). That value is written
into the bundle and therefore into your archive
(`apps/extension/entrypoints/background.ts:164-166`). It never leaves your
...
- **Harness session stores are opened read-only.** Every SQLite connection uses
  ...
  (`crates/chat-stasher/src/main.rs:300,377-378`).
- **`seal` refuses to rename files it cannot justify renaming.** It is gated by
  ...
  the old inode (`crates/chat-stasher/src/main.rs:707-739`).
...
4. **There is no restore command.** The subcommands in this version are `init`,
  ...
  (`crates/chat-stasher/src/main.rs:130-1044`); **a command that puts sessions
...
5. **Search is metadata-only.** `search` walks snapshot/index/tree objects and
   ...
   (`crates/chat-stasher/src/main.rs:479-509`). One qualification, because the
   ...
   **and** from "read it all but could not place every session in time", which
   ...
   (`crates/chat-stasher/src/main.rs:505-509`).
...
7. **Browser-side history backfill runs on seven platforms, recovers text on
   ...
   **ChatGPT**, **DeepSeek**, **Gemini**, **Grok**, **Kimi** and **Claude**
   (`apps/extension/lib/backfill/enumerate.ts:4343-4367`), but on none of them
   ...
   `apps/extension/lib/backfill/enumerate.ts:2908-2929`, `:3087-3090`, `:3513-3518`).
   ...
   (`apps/extension/lib/backfill/enumerate.ts:2560-2587`), while for Claude a branch
   ...
   (`apps/extension/lib/backfill/enumerate.ts:3995-4026`). Grok and Claude are the least
   ...
   rather than archived (`apps/extension/lib/backfill/engine.ts:2437-2450`).
   ...
   (`apps/extension/lib/platform-auth.ts:669-769`;
   `apps/extension/lib/backfill/engine.ts:2237-2277`). On
   **Perplexity** it enumerates your conversations and fetches their content,
   ...
   (`apps/extension/lib/backfill/enumerate.ts:2160-2204`). The user-visible
...
- The behaviour of the extension when the `storage` permission is absent at
  ...
  a real browser (`apps/extension/wxt.config.ts:95-101`).
...
5. Run `verify` (`crates/chat-stasher/src/main.rs:389-427`) rather than assuming
