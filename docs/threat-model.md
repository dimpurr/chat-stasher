...
(`crates/chat-stasher/src/main.rs:220-287`), the schedule itself is visible to
...
3. **The stage directory.** Sealed shards are ordinary files on disk before
   ...
   (`crates/chat-stasher/src/main.rs:288-292`).
...
5. **A directory you exported to.** `chat-stasher export --out <dir>` writes the
   ...
   into a directory you name (`crates/chat-stasher/src/main.rs:593-673`). Unlike
...
(`apps/extension/lib/backfill/engine.ts:1769-1778`, `:2311-2315`), which produces a
...
than being read as complete (`apps/extension/lib/backfill/enumerate.ts:2267-2311`).
...
platform's stable account/org id, computed in the extension with a random
32-byte salt that is generated once per install, kept in `storage.local`, and
never synced (`apps/extension/lib/account-fingerprint.ts:153-188`,
...
- **Harness session stores are opened read-only.** Every SQLite connection uses
  ...
  (`crates/chat-stasher/src/main.rs:352,429-430`).
- **`seal` refuses to rename files it cannot justify renaming.** It is gated by
  ...
  the old inode (`crates/chat-stasher/src/main.rs:763-795`).
...
- **`push` refuses an unprovable empty snapshot.** If the stage holds neither
  ...
  (`crates/chat-stasher/src/main.rs:5744-5840`). It also fails closed when it
  cannot even establish stage safety
  (`crates/chat-stasher/src/main.rs:5716-5723`).
- **A destination that cannot be consulted is not an empty destination.**
  ...
  as many words (`crates/chat-stasher/src/main.rs:4179-4235`).
...
4. **There is no restore command.** The subcommands in this version are `init`,
  ...
  (`crates/chat-stasher/src/main.rs:148-1120`); **a command that puts sessions
   ...
   (`crates/chat-stasher/src/main.rs:384-386,6076-6215`). `export --out <dir>`
   writes **many** sessions to files in one command, laid out as
   `<out>/<machine>/<harness>/<session-id>.jsonl`, and its directory is
   **plaintext** (`crates/chat-stasher/src/main.rs:593-673`) — see exposure 5
...
5. **Search is metadata-only.** `search` walks snapshot/index/tree objects and
   ...
   (`crates/chat-stasher/src/main.rs:531-561`). One qualification, because the
   ...
   **and** from "read it all but could not place every session in time", which
   ...
   (`crates/chat-stasher/src/main.rs:557-561`).
...
7. **Browser-side history backfill runs on seven platforms, recovers text on
   ...
   **ChatGPT**, **DeepSeek**, **Gemini**, **Grok**, **Kimi** and **Claude**
   (`apps/extension/lib/backfill/enumerate.ts:4642-4666`), but on none of them
   ...
   `apps/extension/lib/backfill/enumerate.ts:3203-3224`, `:3382-3385`, `:3808-3813`).
   ...
   (`apps/extension/lib/backfill/enumerate.ts:2806-2833`), while for Claude a branch
   ...
   (`apps/extension/lib/backfill/enumerate.ts:4290-4321`). Grok and Claude are the least
   ...
   rather than archived (`apps/extension/lib/backfill/engine.ts:2617-2630`).
   ...
   `apps/extension/lib/backfill/engine.ts:2417-2457`). On
   **Perplexity** it enumerates your conversations and fetches their content,
   ...
   (`apps/extension/lib/backfill/enumerate.ts:2267-2311`). The user-visible
...
5. Run `verify` (`crates/chat-stasher/src/main.rs:441-479`) rather than assuming
