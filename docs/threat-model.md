...
1. **The outbox window, before delivery.** The extension writes each captured
   ...
   (`apps/extension/entrypoints/background.ts:150-178`). It sits there,
...
`apps/extension/entrypoints/dw-bridge.content.ts:609-635`;
...
(`apps/extension/lib/backfill/engine.ts:1683-1692`, `:2171-2175`), which produces a
...
than being read as complete (`apps/extension/lib/backfill/enumerate.ts:2198-2242`).
...
(`apps/extension/entrypoints/background.ts:165-167`). It never leaves your
...
7. **Browser-side history backfill runs on seven platforms, recovers text on
   ...
   **ChatGPT**, **DeepSeek**, **Gemini**, **Grok**, **Kimi** and **Claude**
   (`apps/extension/lib/backfill/enumerate.ts:4569-4593`), but on none of them
   ...
   `apps/extension/lib/backfill/enumerate.ts:3134-3155`, `:3313-3316`, `:3739-3744`).
   ...
   (`apps/extension/lib/backfill/enumerate.ts:2737-2764`), while for Claude a branch
   ...
   (`apps/extension/lib/backfill/enumerate.ts:4221-4252`). Grok and Claude are the least
   ...
   `apps/extension/entrypoints/background.ts:927-987`). Kimi's routes, by contrast, were measured in a logged-in session,
   ...
   rather than archived (`apps/extension/lib/backfill/engine.ts:2477-2490`).
   ...
   `apps/extension/lib/backfill/engine.ts:2277-2317`). On
   **Perplexity** it enumerates your conversations and fetches their content,
   ...
   (`apps/extension/lib/backfill/enumerate.ts:2198-2242`). The user-visible
