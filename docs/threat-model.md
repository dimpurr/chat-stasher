...
(`apps/extension/lib/backfill/engine.ts:1357-1423`, `:1784-1806`), which produces a
...
- **The host manifest names exactly one allowed extension id.** Chrome's
  ...
  `apps/extension/wxt.config.ts:114-120`).
...
7. **Browser-side history backfill runs on seven platforms, recovers text on
   ...
   **ChatGPT**, **DeepSeek**, **Gemini**, **Grok**, **Kimi** and **Claude**
   (`apps/extension/lib/backfill/enumerate.ts:4249-4276`), but on none of them
   ...
   `apps/extension/lib/backfill/enumerate.ts:2922-2925`, `:3087-3090`, `:3513-3518`).
   ...
   ending at a parent the response does not carry is the branch root — the shared
   tree-root id every real body omits, measured 2026-09-24 — so the body **is**
   archived, and only a missing newest message or a cycle in the parent links is not
   (`apps/extension/lib/backfill/enumerate.ts:3907-3932`). Grok and Claude are the least
   ...
   rather than archived (`apps/extension/lib/backfill/engine.ts:2053-2088`).
   ...
   `apps/extension/lib/backfill/engine.ts:1949-2001`). On
   **Perplexity** it enumerates your conversations and archives
   **none of them** (`apps/extension/lib/backfill/enumerate.ts:4249-4276`). The user-visible symptom of
...
- The behaviour of the extension when the `storage` permission is absent at
  ...
  a real browser (`apps/extension/wxt.config.ts:83-89`).
