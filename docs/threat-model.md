...
(`apps/extension/lib/backfill/engine.ts:808-873`, `:1183-1205`), which produces a
...
7. **Browser-side history backfill runs on seven platforms, recovers text on
   ...
   **ChatGPT**, **DeepSeek**, **Gemini**, **Grok**, **Kimi** and **Claude**
   ...
   organization's history (`apps/extension/lib/backfill/claude-org.ts:195-249`).
   ...
   `apps/extension/entrypoints/background.ts:790-811`). Kimi's routes, by contrast, were measured in a logged-in session,
   ...
   rather than archived (`apps/extension/lib/backfill/engine.ts:1417-1452`).
   ...
   `apps/extension/lib/backfill/engine.ts:1313-1365`). On
