...
| **Perplexity** | Conversation-list requests **only** | **Nothing.** Not one conversation body is requested or delivered (`apps/extension/lib/backfill/enumerate.ts:4343-4367`) |
...
conversation (`apps/extension/lib/contract.ts:367-399`) — but the sources
...
- **`push` refuses an unprovable empty snapshot.** If the stage holds neither
  ...
  (`crates/chat-stasher/src/main.rs:5228-5321`). It also fails closed when it
  cannot even establish stage safety
  (`crates/chat-stasher/src/main.rs:5200-5207`).
- **A destination that cannot be consulted is not an empty destination.**
  ...
  as many words (`crates/chat-stasher/src/main.rs:3932-3988`).
...
4. **There is no restore command.** The subcommands in this version are `init`,
  ...
  (`crates/chat-stasher/src/main.rs:130-1044`); **a command that puts sessions
   ...
   (`crates/chat-stasher/src/main.rs:332-334,5557-5675`). `export --out <dir>`
...
7. **Browser-side history backfill runs on seven platforms, recovers text on
   ...
   **none of them** (`apps/extension/lib/backfill/enumerate.ts:4343-4367`). The user-visible symptom of
