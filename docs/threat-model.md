...
2. The extension writes that text, as a JSON bundle, into its **own IndexedDB
   ...
   `apps/extension/entrypoints/background.ts:268-285`).
...
1. **The outbox window, before delivery.** The extension writes each captured
   ...
   (`apps/extension/entrypoints/background.ts:148-176`). It sits there,
...
(`apps/extension/entrypoints/background.ts:163-165`). It never leaves your
...
7. **Browser-side history backfill runs on seven platforms, recovers text on
   ...
   **ChatGPT**, **DeepSeek**, **Gemini**, **Grok**, **Kimi** and **Claude**
   ...
   `apps/extension/entrypoints/background.ts:925-969`). Kimi's routes, by contrast, were measured in a logged-in session,
