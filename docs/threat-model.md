...
1. **The outbox window, before delivery.** The extension writes each captured
   ...
   (`apps/extension/entrypoints/background.ts:136-164`). It sits there,
...
`apps/extension/entrypoints/dw-bridge.content.ts:598-624`;
...
backfill paths and nothing else, and holds no copy (`apps/extension/lib/platform-auth.ts:216-248`);
...
and nothing else (`apps/extension/lib/platform-auth.ts:513-575`;
...
(`apps/extension/lib/backfill/engine.ts:808-873`, `:1204-1226`), which produces a
...
(`apps/extension/entrypoints/background.ts:151-153`). It never leaves your
...
7. **Browser-side history backfill runs on seven platforms, recovers text on
   ...
   **ChatGPT**, **DeepSeek**, **Gemini**, **Grok**, **Kimi** and **Claude**
   (`apps/extension/lib/backfill/enumerate.ts:4048-4075`), but on none of them
   ...
   rather than truncated (`apps/extension/lib/backfill/enumerate.ts:3711-3731`).
   ...
   (`apps/extension/lib/backfill/enumerate.ts:2388-2415`, `:3503-3559`). Grok and Claude are the least
   ...
   `apps/extension/entrypoints/background.ts:794-815`). Kimi's routes, by contrast, were measured in a logged-in session,
   and its requests carry the page's own login token, read at request time and
   held in memory only (`apps/extension/lib/platform-auth.ts:216-248`); a body
   response that admits it is incomplete is refused and listed as a failure
   rather than archived (`apps/extension/lib/backfill/engine.ts:1438-1473`).
   ...
   (`apps/extension/lib/platform-auth.ts:513-575`;
   `apps/extension/lib/backfill/engine.ts:1334-1386`). On
   **Perplexity** it enumerates your conversations and archives
   **none of them** (`apps/extension/lib/backfill/enumerate.ts:4048-4075`). The user-visible symptom of
