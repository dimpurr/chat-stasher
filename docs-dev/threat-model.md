...
having conversations. If you archive on a schedule, the schedule itself is
visible to them as a pattern of writes. If you archive manually, the write
times are a usage log.
...
- **`push` refuses an unprovable empty snapshot.** If the stage holds neither
  ...
  (`crates/chat-stasher/src/main.rs:6664-6760`). It also fails closed when it
  cannot even establish stage safety
  (`crates/chat-stasher/src/main.rs:6636-6643`).
- **A destination that cannot be consulted is not an empty destination.**
  ...
  as many words (`crates/chat-stasher/src/main.rs:4989-5045`).
...
4. **There is no restore command.** The subcommands in this version are `init`,
   ...
   (`crates/chat-stasher/src/main.rs:415-417,6996-7135`). `export --out <dir>`
...
5. **Search remains metadata-only; full-text indexing is an explicit local
   ...
   (`crates/chat-stasher/src/fts.rs:1-6,557-670,673-687,823-828`; `crates/chat-stasher/src/main.rs:7297-7529`). One qualification, because the
