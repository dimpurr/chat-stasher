...
- **The optional local full-text index is plaintext.** `index build` reads
  ...
  (`crates/chat-stasher/src/fts.rs:1-6,557-670,673-687`; `crates/chat-stasher/src/main.rs:7381-7613`).
...
2. **Queue on your machine.** The extension writes that text, as a JSON bundle,
   ...
   (`apps/extension/entrypoints/background.ts:313-330`).
3. **Deliver to the local host.** The extension hands the bundle to a Native
   ...
   `ingest` (`crates/chat-stasher/src/nativehost.rs:1499-1526`).
...
is, the conversation itself (`apps/extension/entrypoints/background.ts:203-206`;
...
(`crates/chat-stasher/src/activity.rs:1999-2049`). A project name is a label from
...
- The `<scope>` part of that key is your **account identifier on that platform**
  ...
  (`apps/extension/entrypoints/background.ts:1619-1662` — the identity itself is
  read by `apps/extension/lib/contract.ts:1271-1287`; the `default` fallback is on
  the `||` at `apps/extension/entrypoints/background.ts:1662`). It is used to
  ...
  (`apps/extension/entrypoints/background.ts:1314-1374`, `:1532-1606`).
- **On claude.ai the scope is not read from a response body: it is the
  ...
  **in the claude.ai page**, over the channel the backfill already uses, and only
  ...
  `apps/extension/entrypoints/background.ts:1314-1374`). A record an **earlier**
  ...
  (`apps/extension/entrypoints/background.ts:1881-1962`). A backfill for a *second*
  ...
  `apps/extension/entrypoints/background.ts:929-959`). Collapsing a title cannot
  ...
  `apps/extension/entrypoints/background.ts:1881-1962`). The host archive is
  ...
  (`apps/extension/lib/backfill/alarm.ts:1101-1111, 1160-1205`;
  `apps/extension/lib/popup-view.ts:1616-1703`).
...
(`apps/extension/lib/backfill/engine.ts:2533-2551`). That produces a request pattern
...
(`apps/extension/lib/backfill/enumerate.ts:4630-4661`;
`apps/extension/lib/backfill/engine.ts:2621-2694`.) The
...
(`apps/extension/lib/backfill/enumerate.ts:3688-3816`; `apps/extension/lib/backfill/engine.ts:2650-2690`).
...
half-there (`apps/extension/lib/backfill/engine.ts:2872-2885`). Grok
...
(`apps/extension/lib/backfill/engine.ts:2069-2149`). And because no source says
...
(`crates/chat-stasher/src/main.rs:2029-2067`, `:2136-2142`). Draining each
