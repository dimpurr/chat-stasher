...
describe('W86-A · the rotation survives a registry that reorders under it', () => {
  it('🔴 a capture between every wake must not let one scope take them all', async () => {
    ...
    //    makes. 🔴 W86c §1 · the tie-break among never-served rows moved: they
    //    rank at the *back* of the rotation now, so a fresh registry is served in
    //    its own order ([A,B,C]) rather than re-ordering the newcomer group first.
